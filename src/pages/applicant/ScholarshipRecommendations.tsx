...
interface Props {
    /** When provided the component uses this ID directly and skips the summary fetch. */
    applicationId?: number;
    /** When true, the outer page header/wrapper is omitted (for use inside a tab). */
    embedded?: boolean;
}

const ScholarshipRecommendations: React.FC<Props> = ({ applicationId: applicationIdProp, embedded = false }) => {
    const { token } = useAuth();

    const [applicationId, setApplicationId] = useState<number | null>(applicationIdProp ?? null);
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (applicationIdProp !== undefined) {
                // Application ID already known — skip the summary fetch
                appId = applicationIdProp;
                ...
                appId = activeApp.application.id;
                setApplicationId(appId);
    ...
    if (loading) {
        return (
            <div className={`${embedded ? 'py-5' : 'min-vh-100 bg-light'} d-flex align-items-center justify-content-center`}>
    ...
    const content = (
        <div className={embedded ? '' : 'container-fluid px-4 py-4'}>
            ...
            {/* Evaluation summary — only shown in standalone mode (embedded view already has eval in Overview tab) */}
            {!embedded && evaluation && (
